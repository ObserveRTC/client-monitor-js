import { IceTraversalDetector } from "../../src/detectors/IceTraversalDetector";
...
class MockClientMonitor {
    ...
    raiseIssue() {
        throw new Error('IceTraversalDetector must not raise issues');
    }
...
describe('IceTraversalDetector', () => {
    let detector: IceTraversalDetector;
    ...
    beforeEach(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        detector = new IceTraversalDetector(mockPeerConnection as any);
    });

    it('should create detector with correct name', () => {
        expect(detector.name).toBe('ice-traversal-detector');
    });
