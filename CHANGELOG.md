...
-   **Score reasons ship — and are controllable.** `DefaultScoreCalculator` now implements the per-entity encoder methods of the `ScoreCalculator` interface, so `scoreReasons` actually reach the peer-connection and track sample entries with the default calculator (previously the optional methods were unimplemented and the field was always empty). Attribution is per entity by construction: track-caused reasons ship on the track's own sample, the peer connection's on its own. The new **`sendScoreReasonsToServer`** config flag (default `true`; only an explicit `false` disables) drops the reasons from the wire without touching the scores or the realtime `'score'` event.

### Other changes

-   New monitor events: `'blocked-transport'`, `'no-available-ice-candidate'`. New config blocks (existing convention: `null` disables construction, omitted applies defaults): `blockedTransportDetector`, `noAvailableIceCandidateDetector`. The two new issue types joined the `ClientMonitorIssue` / `ClientMonitorResolvedIssue` unions and `isClientMonitorIssue`.
