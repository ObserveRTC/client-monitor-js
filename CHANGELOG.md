...
-   **Two detectors, one question.** `OutboundFrameSupplyDetector` and `InboundFrameSupplyDetector` both ask whether whatever supplies a track's frames is delivering what it should. Outbound the supply is the capture device and the bar is `getSettings().frameRate` (`capture-bottleneck`); inbound it is the decoder and the bar is the rate frames actually arrived at (the new `decoder-bottleneck`). They are separate classes because the guards, the stats they read and the actions they suggest have nothing in common past the arithmetic; the arithmetic itself — a rolling window of starving intervals — lives in a shared `StarvingWindow`, and the shared payload shape in `FrameSupplyIssuePayload`.

-   **New issue: `decoder-bottleneck`.** Frames arrived and the decoder did not turn enough of them into pictures, over a rolling window. This is deliberately narrow: the bar is the *arrival* rate, so a stream throttled to 5fps that decodes cleanly is silent, and frames that never arrived remain the network's story. It complements `DecoderPerformanceDetector`, which asks whether decoding *cost* too much over consecutive ticks — the window is what catches a decoder that stumbles intermittently, the same shape that hid the failing camera. Both can fire on a badly overloaded decoder; that is the honest answer when both are true. Configure with `inboundFrameSupplyDetector`, or pass `null` to leave it off.

-   **`encoder-bottleneck` stays in the same detector**, on the outbound instance only. It needs exactly what the capture check already computed — the source frame rate — and exactly the same track guards, so it reads them rather than duplicating them. It fires only while the source is at or above `minProducedFps` (the same floor the capture half falls back on): blaming the encoder for frames it was never handed is the misattribution the pair exists to avoid. Two behaviour changes: it now stays silent on a paused sender, and it *is* judged on screen shares, where the capture half is not — an encoder falling behind a screen share is as real as any other.

#### The capture-side behaviour changes that came with it

-   **`EncoderPerformanceDetector` is split out of `OutboundFrameSupplyDetector`.** The two halves were answering different questions on different clocks, and the codebase already had the shape they belong in — the four detectors now form a 2×2 where each cell has one job:

    | | frames going missing (average over a duration) | the stage cannot keep up (consecutive ticks) |
    |---|---|---|
    ...
    `EncoderPerformanceDetector` is the exact mirror of `DecoderPerformanceDetector`: same question, same unit, same shape of signal. New config block `encoderPerformanceDetector`; `outboundFrameSupplyDetector` drops to two fields (`durationInMs`, `captureFpsRatioThreshold`). Each detector file exports its own config *type*, so a detector can be read on its own; the default *values* stay in `ClientMonitor`'s `detectorDefault(...)` calls with every other detector's.

-   **The units go back to matching what each number is for.** Ticks and durations are not two spellings of the same thing. A tick count is a *confidence* floor — "two independent stats reads agreed", which is two samples whatever the collecting period is — and every encoder signal is a per-interval ratio a single read can get wrong. A duration is a *persistence* bar: the capture device stayed short long enough to matter. So `minConsecutiveTicks: 2` returns to the encoder, `durationInMs` stays on capture, and the previous release's blanket "durations everywhere" is corrected. The same reasoning is why `JitterBufferStressDetector`, `DecoderPerformanceDetector` and `StuckDecoderDetector` keep their tick counts.

-   **The capture → encoder chain is now read from the issue.** `EncoderPerformanceDetector` checks `ClientMonitor.isIssueActive('capture-bottleneck-track-<id>')` rather than sharing a private field, which makes the dependency explicit and inspectable — a quiet encoder is explained by an issue anyone can see. `OutboundTrackMonitor` registers the capture detector first and `Detectors.update()` preserves order, so the verdict is same-tick. Dropped with the merge: the rule that withheld every encoder verdict until the first capture window completed, which cost up to 15s at the start of each track and bought nothing — the encoder comparison is against what the source *delivered*, so a starving source with an encoder keeping up never tripped it anyway.

-   **Each ratio says which half it belongs to.** The outbound detector's bare `fpsRatioThreshold` sat next to `encodeFpsRatioThreshold`, so nothing in the name said it was the capture one; it is now `captureFpsRatioThreshold`, the name it had before this refactoring. The inbound one becomes `decodeFpsRatioThreshold` for the same reason, leaving the three ratios named for the three stages: capture, encode, decode.

-   **One `durationInMs` governs both halves, replacing `windowInMs` and `minConsecutiveTicks`.** The capture average is taken over it and the encoder condition must hold continuously for it. `minConsecutiveTicks` was the last tick count left in these detectors and had the same flaw every other one had: "two consecutive ticks" is four seconds at a 2s collecting period and ten at a 5s one. The outbound config is down to five fields and the inbound one to three. On the payloads, `windowSeconds` becomes `averagedOverInMs` and the encoder's `consecutiveTicks` becomes `heldForInMs`.

    Note the effect on the encoder's response time: at a 2s collecting period it now needs 15s of sustained shortfall rather than 4s. That filters transients, and it costs latency in any adaptation loop reacting to `encoder-bottleneck`. Lower `durationInMs` if you want it back — it shortens the capture average too, which is the point of there being one number.

-   **The capture and encoder checks are chained and mutually exclusive.** The source question comes first: if the capture device is short, that is the whole answer and the encoder is not judged at all — an encoder handed too few frames has nothing to answer for. Only once a completed window finds the source delivering does the encoder question apply. At most one of `capture-bottleneck` and `encoder-bottleneck` is ever active, and neither fires before the first window completes. This replaces the old `minSourceFps` bar, which tried to answer the same question with a fabricated constant.

-   **No baseline, no judgement.** `capture-bottleneck` no longer falls back to an absolute frame-rate floor when the browser reports no `getSettings().frameRate`. Nothing is substituted for a missing baseline: without a stated rate there is nothing for the measurement to fall short *of*, so the check stays quiet. `minProducedFps` is gone from the outbound config; the inbound one's equivalent is renamed `minReceivedFps`, because there the baseline *is* measured (the arrival rate) and the number only refuses to compute a ratio over a handful of frames.

-   **`cpuLimitationShareThreshold` is now `number | null` and defaults to `null`** — the browser's own CPU-limitation share is ignored unless you configure it. `CpuPerformanceDetector` already reports CPU limitation as `cpulimitation`, and the useful thing to do with the two is correlate them: both firing together is evidence the encoder is CPU-bound. That inference only holds while `encoder-bottleneck` is derived without reading the same signal — wire it in here too and the correlation is tautological. Set `0.3` for the previous behaviour.

-   **The capture device is averaged over a window and compared once.** Add up the frames the source delivered and the time it had; when `windowInMs` (15s) of time has accumulated, compare the average against `getSettings().frameRate` and judge it against `fpsRatioThreshold` (0.9). Two running totals, no history buffer. Averaging is what catches a camera degrading in bursts — 150 frames per 5s tick becomes 132, back to 150, then 97, so most individual ticks look fine while the 15s average reads 26.3fps against a configured 30 — and it weights how far the source fell short rather than merely how often. On the captured failure it raises at t=45s, 30 seconds before the camera stopped. **Replaces the earlier rolling ring buffer of "starving intervals"**, which needed three parallel arrays, a second duration threshold and a bounded-capacity ring to reach the same verdict on the same tick.

-   **Superseded: starving *time* summed over a rolling window** (`windowInMs`, default 120s; `minStarvingTimeInMs`, default 15s) instead of counting a consecutive run of ticks. Two changes in one: the window, because a camera that is failing rather than merely busy produces starving intervals *interleaved* with healthy ones — 150 frames per 5s tick becomes 132, then 150 again, then 97 — so the consecutive counter reset before it ever reached its threshold; and the duration, because a tick count means six seconds at a 2s collecting period and thirty at 10s, so the same configuration would have judged two deployments differently. Each interval now contributes the time it actually measured. On the captured failure the window raises 99 seconds before capture stopped; the old rule stayed silent until after.

-   **The capture ratio moves from `0.5` to `0.8`, which makes `capture-bottleneck` more sensitive.** Half the configured frame rate is below every dip a degrading camera produces: on the captured session the dips ran 26.4, 19.4, 24.6, 17.6 and 15.2fps against a 30fps request, all above the old bar, so nothing was reported until the camera stopped outright — long past the point at which anything could still be done about it. **Treat the issue as observation-grade at the new default**: the numbers come from two sessions (one failure, one control, one user, one camera model), they are not a validated false-positive rate, and low light is the case most likely to trip them — many webcams settle at 15fps in a dim room while `getSettings().frameRate` still reports 30. Set `fpsRatioThreshold: 0.5` for the old behaviour.

    **Known cost of the shipped defaults.** `0.8` paired with `minStarvingTimeInMs: 30000` is conservative enough that on the captured failure it reports only *after* the camera stopped: 30s is more starving time than that degradation accumulates while still delivering, since at `0.8` only three of its five dips count as starving at all. `minStarvingTimeInMs: 15000` moves the fire 20 seconds earlier, at 15.2fps, which is the case the rolling window exists for. Both behaviours are pinned in `tests/detectors/OutboundFrameSupplyDetector.replay.spec.ts`, so a future change to either number shows up as a test change rather than a silent one.

-   **A missing capture frame rate is no longer substituted with a fabricated fallback**, and the rate is never backfilled from `mediaSource.framesPerSecond`. The browser's own figure is coarse and smooths this exact stutter away — over the failing session it reported `30` on ticks that actually delivered 132 and 133 frames. No counter means no judgement.

-   **`MediaSourceMonitor.sourceFps` is `undefined`, not `0`, when the frame counter resets.** `positiveDelta` clamps a counter reset to `0`, so replacing a track (`replaceTrack`, a device switch) briefly presented a healthy new camera as one delivering zero frames per second.

-   **Four situations are no longer judged**, because a low frame rate in them is legitimate rather than a fault anyone can act on. This narrows `capture-bottleneck`:
    -   **Screen shares.** Frame rate there is content-driven — a still document delivers nothing — and the old docstring listed "a screen share of static content" as something it reported, which in practice was a standing false positive. Declare a moving surface that should still be watched with `setOutboundTrackContext(trackId, { contentType: 'camera' })`.
    -   **Backgrounded tabs** (`ClientMonitor.activeTab === false`), where the browser throttles capture and playout along with everything else.
    -   **Paused senders and consumers**, and tracks that are not live, enabled and unmuted.
    -   **Collection gaps**, settings changes and counter resets, each of which discards the window rather than interpreting it — ticks stopping is not frames stopping. The gap threshold is **derived from `collectingPeriodInMs`** (`maxTickGapInMs(period)` in `utils/common`) rather than configured: a fixed millisecond value means something different at every collecting period, so there is no knob for it.

-   **`CaptureBottleneckIssuePayload` is now an alias of the shared `FrameSupplyIssuePayload`** that `decoder-bottleneck` also uses. **`consecutiveTicks` is replaced by `starvingTimeInMs`** — it had not counted a consecutive run since the window landed, and a tick count is not comparable across collecting periods. Added alongside it: `windowSeconds`, `worstSourceFps` (how deep the dips went), `msSinceFirstStarvingTick`, and `trackReadyState`/`trackMuted` — the last pair because "live, unmuted, no frames" is the signature of a pipeline failing upstream, where an unplugged or OS-muted device reports `ended`/`muted` and belongs to `CaptureFailureDetector` instead. `EncoderBottleneckIssuePayload.consecutiveTicks` is unchanged; there it really is a consecutive run.

### Replay harness CLI

-   **`npm run replay -- <stats.jsonl>`** runs a captured session through a real `ClientMonitor` on a virtual clock and prints every detector fire as NDJSON on stdout: one `{"record":"issue",...}` per fire with the issue's own timestamp plus the `tick`/`tickTimestamp` locating it in the input, then a `summary` record with per-type counts. Warnings and progress go to stderr, so the output pipes directly into `jq` or a corpus runner sweeping thresholds across many sessions. `--only`, `--config`, `--pretty`, `--updates`, `--real-time` and `--no-summary` are documented under `--help`. Opt-in detectors are enabled by default under replay. Reads from stdin with `-`.

    The CLI builds on the existing `StatsReplayer` / `replayFixture` sandbox, and needs no new dependency: `scripts/replay.mjs` compiles `scripts/replay.ts` to CommonJS under `.replay/` and runs it. `tests/fixtures/degrading-camera.jsonl` is a new capture of the interleaved-degradation shape, replayed as a spec in `tests/detectors/CaptureWindowReplay.spec.ts` so the capture window is covered through the real stats path — raw `getStats()` dictionaries, `MediaSourceMonitor`, `getSettings()` — and not only through mocks.

### CPU performance detector: frame-arrival burst guard

-   **Bursty frame arrival no longer reads as CPU limitation (LIV-1595).** The inbound decoded/received frames ratio false-triggered `cpulimitation` on one-interval arrival spikes — a simulcast layer switch, keyframe recovery or post-stall queue flush delivers a pile of frames inside a single collect interval, the decoder trails that spike for exactly that interval, and the ratio dips below `alertOn` while the machine is idle (observed as repeated one-tick dips on an idle M4 Pro, each coinciding with a `VIDEO_RESOLUTION_CHANGED` and recovering to ~1.0 on the next tick). `CpuPerformanceDetector` now keeps a smoothed (EWMA, α = 0.3) frames-received-per-interval baseline per inbound video ssrc and **skips the ratio judgement** on any interval whose arrival exceeds `frameArrivalBurstFactor` × that baseline (new field on `incomingDecodedFramesRatioThresholds`, default `2.5`), as well as on a track's first interval (no baseline yet — fresh consumers routinely start with a keyframe burst). Sustained decoder starvation still alerts, because its low ratio persists across ordinary-arrival intervals; a sustained *higher* arrival rate is judged again as soon as the baseline adapts (a few intervals). Set `frameArrivalBurstFactor` to `undefined` to restore the old judge-every-interval behavior. Baselines are rebuilt from the tracks seen on each tick, so entries of removed tracks never linger.

### Schema

-   **ClientSample schema `3.6.0`.**
    -   `scoreReasons` on the client, peer-connection and track sample entries is a `Record<string, number>` — the score reasons attached to that entity's calculated score, each key carrying the points it subtracted.
...
### Score reasons are shipped once, by the component that caused them

-   **The client sample no longer ships the aggregated score reasons.** `ClientMonitor.scoreReasons` is the sum of every component's reasons below it (each peer connection's own, plus each track's), and `createSample()` was shipping that aggregate on the client entry — so a single inbound track pixelating produced `pixelated-video` on the track entry *and* on the client entry of the same sample. That duplicated every reason on the wire and read as though the client itself were the thing pixelating or losing packets. It also invited misreading, since the client score is a smoothed weighted aggregate and does not equal `5 - sum(reasons)`: a sample could show a recovered client score of ~5 next to a `high-packetloss` entry inherited from a component, which looks like a stale reason and is really a scope error.

    Now every entity ships only what it is itself responsible for. The client score subtracts nothing of its own, so its sample entry carries no reasons at all; peer-connection and track entries are unchanged. **No information is lost** — a server reconstructs the client-level view in post-analysis by re-aggregating the components of the same sample, which it must do anyway to attribute a degradation. The live view is also unchanged: `ClientMonitor.scoreReasons` and the `'score'` event still carry the full aggregate for applications reacting in real time.

-   **Score and score reasons are now kept separate on the client.** `ClientMonitor.scoreReasons` means what it means on every other monitor — this entity's *own* subtractions — and is what the sample ships; there are none today, so it stays undefined. The aggregated view moved to the `'score'` event's `currentReasons`, which still carries every component's reasons summed by key, so applications reacting live are unaffected. `ClientMonitor.setScore(score, ownReasons, aggregatedReasons)` takes the two separately. If a client-level penalty is ever added it lands on `scoreReasons` like any other component's and ships automatically, with no extra field.

-   Fixed in the README along the way: the `'score'` event payload was documented as carrying `scoreReasons`; the field is and was `currentReasons`.

### Video pixelation scoring

-   **`low-bitrate-per-pixel` is gone; `pixelated-video` replaces it, driven by the inbound `qpSum`.** The old reason compared `bitrate / (width * height * measuredFps)` against a fixed per-codec floor and was wrong twice over: the floor ignored resolution (required bitrate scales as roughly `pixels^0.75`, so one floor cannot fit 180p and 1080p — in practice it demanded ~1 Mbps at 360p30, above what healthy real-time streams send, and a loss-free 640x358@30 vp8 stream at 500 kbps took the **fully saturated** penalty every sample), and dividing by *measured* fps meant a track halving its frame rate doubled its bits-per-pixel and shed the penalty — the metric rewarded dropping frames.

    Bitrate cannot answer the question at all: the same 500 kbps is generous for a static talking head and starvation for a fast pan. QP — the encoder stating how coarsely it had to quantize — measures the blockiness and detail loss the viewer actually sees, so `pixelated-video` ramps from the codec's activation QP to its saturation QP (`VIDEO_QP_THRESHOLDS`). Where the browser does not report `qpSum` for the codec in use, **the reason is absent entirely** rather than modelled from bitrate.

-   **`InboundRtpMonitor.avgQpPerFrame`** (and `deltaQpSum`) derive the mean quantizer of the frames decoded in the interval, mirroring what `OutboundRtpMonitor` already did for the sending side. It resets rather than carrying a stale average forward when no frames were decoded, so a frozen or dry track stops reporting the quantizer of media that is no longer on screen.

-   **Motion class selects the QP band, and is application-declared.** The same quantizer is not equally visible on all content: movement masks compression artifacts, static content shows every blocked edge. `VIDEO_QP_THRESHOLDS` is indexed `[codec][motionType]` with the bands written out literally — they are constant for the life of a track, so nothing is recomputed per tick, every number is directly tunable, and none can silently fall outside its codec's range (H.264's high-motion band has to stay under 51). The bands run the opposite direction to bitrate, since high-motion content needs more bits to reach a given QP yet tolerates a higher one once there. Set it with `InboundTrackMonitor.setContext({ motionType })` or `ClientMonitor.setInboundTrackContext(trackId, { motionType })`, the latter working from signaling before the track has manifested. Undeclared, screen share is judged as `lowmotion` (blocked text is a hard failure) and everything else as `standard`.

-   **QP thresholds are per codec and configurable.** QP scales are not comparable as fractions of their ranges (H.264 0-51, VP8 0-127, VP9 0-255, AV1 0-255), so `VIDEO_QP_THRESHOLDS` carries an activation/saturation pair per codec; an unrecognised codec yields no judgement. Every penalty ramp on `DefaultScoreCalculator` is now a mutable static rather than `readonly`. The shipped values are literature starting points, not measurements of any deployment.

-   **Removed as dead code:** `BPP_RANGES`, `expectedVideoBitrate`, `VIDEO_BITRATE_EXPECTATION`, `BaseVideScoreContext`, `MAX_VIDEO_SCORE`, `MIN_VIDEO_SCORE` and `calculateBaseVideoScore`. The motion-type API survives, but now scales the QP band by how visible artifacts are on that content rather than guessing how many bits it needs.

### Application-declared track context

-   **`ClientMonitor.setInboundTrackContext(trackId, context)` and `setOutboundTrackContext(trackId, context)`** replace the per-field setters. Everything the application knows about a track and the stats never reveal now travels through one call per direction, taking a partial object rather than a positional value:
    ...
    On the monitors themselves, `InboundTrackMonitor.setContext()` and `OutboundTrackMonitor.setContext()` replace `setContentType()` / `setMotionType()`. **Removed:** `ClientMonitor.setTrackContentType()`, `ClientMonitor.setTrackMotionType()`, `Inbound/OutboundTrackMonitor.setContentType()`, `InboundTrackMonitor.setMotionType()`. All four were introduced in this same unreleased version, so nothing published is affected. The `contentType` and `motionType` *fields* are unchanged and still directly assignable.

-   **Declarable before the track exists, in both directions.** The pending mechanism the content type already had (declare from signaling, applied by whichever peer connection first manifests the track) now covers every context field and is split per direction: `takePendingInboundTrackContext` / `takePendingOutboundTrackContext`. The previous single pending map was keyed by track id across both directions, so a send and a receive track sharing an id could take each other's declaration; they are now separate maps. No timers, no cleanup — an entry for a track that never appears is one map entry for the monitor's lifetime.

-   **Contexts merge, they do not replace.** Fields omitted from the argument keep whatever was declared before, on a live monitor and in the pending state alike, so the pieces can be declared as they become known: a content type from signaling survives a later call that only attaches the video element. An explicit `undefined` means "not declared here", not "reset" — assign the field directly to clear it. This is the behaviour a positional setter could not express and the reason the API is a bag rather than four methods.

-   **One `TrackContentType`, living on `TrackMonitor`.** `InboundTrackContentType` and `OutboundTrackContentType` were the same two members (`'camera' | 'screenshare'`) declared twice, which made the identical value look like two types and forced anything handling a `TrackMonitor` generically to accept a union of duplicates. They are merged into **`TrackContentType`** in `monitors/TrackMonitor.ts`, beside the `TrackMonitor` union itself — the direction-neutral home. The old names are gone from the public exports; both were added in this same unreleased version. What differs between the directions is not the content type but how it is *arrived at* (auto-detected from `displaySurface` when sending, declared when receiving), and that lives in the monitors, not in the type.

-   **The two directions carry different fields, by design.** `InboundTrackContext` has `contentType`, `motionType`, `presentedResolution` and `videoTag`; `OutboundTrackContext` has only `contentType`. Motion class and presentation describe how a track is *watched*, which the sender does not know — so they are absent from the outbound type rather than accepted and ignored.

-   **`pixelated-video` is now judged at the size the picture is actually shown.** The quantizer says how coarsely a frame was coded; it says nothing about how large those coded blocks end up on the viewer's screen, and that is the other half of whether anyone can see them. The same QP is punishing blown up to full screen and nearly invisible in a grid thumbnail — and a video call is full of both at once. Where the application has declared `presentedResolution` and the stats report a decoded resolution, the penalty is scaled by the linear ratio between them:
    ...
    Judging at display size rather than decode size is what reference metrics do — VMAF scales to the display before scoring — and this is the cheap version of it. The ratio is taken from the **areas**, so a presented box whose proportions differ from the frame's does not read as magnification on width alone, and it is clamped because it is unbounded in both directions: 180p on a 4K screen is a factor of ~10.7, 1080p in a 96px avatar is ~0.07, and neither extreme is worth the score swing. `PIXELATION_MAGNIFICATION_MIN` (0.5) and `PIXELATION_MAGNIFICATION_MAX` (2.0) are mutable statics like every other ramp; set both to `1.0` to disable the adjustment.

    **Note the ceiling on this reason doubles**: `pixelated-video` could previously subtract at most 1.0 and can now subtract 2.0, the same as `frozen-video`. A saturated quantizer magnified 2x is a two-point experience. A *clean* picture is unaffected however far it is magnified — the penalty is zero before the multiplication and zero after — so this only ever amplifies blockiness that was already being reported.

    **No presented resolution, no adjustment.** Nothing is substituted for a missing number: without both sides of the ratio the quantizer is judged alone, exactly as it was.

-   **`presentedResolution` is declared in device pixels, or derived from a `videoTag`.** Hand over the element and it is re-measured every tick, so going full-screen or resizing a panel is picked up rather than frozen at whatever the layout was when the track appeared. The derivation reads the element's **layout box** (`clientWidth`/`clientHeight` × `devicePixelRatio`) and fits the frame's aspect ratio into it the way `object-fit: contain` does — never `videoWidth`/`videoHeight`, which are the *intrinsic* decoded size and identical to what the stats already report, so measuring with those would have made every magnification exactly 1 and the feature a no-op. An application using `object-fit: cover`, which crops rather than letterboxes, should declare `presentedResolution` itself. A non-positive box (`display: none`, detached, not yet laid out) leaves the previous value alone.

### Mediasoup pause & data-channel wiring

-   **`ClientMonitor.activeTab` and the tab-visibility watcher.** The monitor now subscribes to `document.visibilitychange` (config flag `watchTabVisibility`, on by default) and keeps the new `ClientMonitor.activeTab` boolean current. It defaults to `true` and stays `true` when the watcher is disabled or no `document` exists (SSR, tests, workers) — `false` always means the tab really is hidden. Each transition is captured as a `TAB_VISIBILITY_CHANGED` client event (`{ visible }`), customizable via `clientEventPayloadProvider.createTabVisibilityChangedEventPayload`. Browsers throttle background tabs — timers fire late, rendering stops, decoding slows — so the detectors whose signals that corrupts stand down while the tab is hidden: `CpuPerformanceDetector` (a hidden tab is not a CPU problem), `DecoderPerformanceDetector`, `StuckDecoderDetector`, `PlayoutDiscrepancyDetector` (rendering stops entirely) and `FreezedVideoTrackDetector` (which also swallows freeze counters accumulated in the background so returning to the tab does not replay them). Each resolves its active issue with `tab in background` when the tab hides mid-episode.
-   **Paused producers and consumers no longer look like dry tracks.** `MediasoupTransportBinding` now mirrors pause state onto the track monitors, keeping the two kinds of silence distinct: a producer's `paused` lands on `OutboundTrackMonitor.paused` (new flag — the sender stopped for everyone), and a consumer's `paused` lands on the new `InboundTrackMonitor.paused` (this receiving leg opted out; the producer may still feed everyone else). `InboundTrackMonitor.remoteOutboundTrackPaused` keeps its original meaning — the remote producer went silent for everyone — and stays application-set, since mediasoup only delivers that fact over the application's own signaling. Both flags are synced on the pause/resume observer events and re-synced on every stats tick, because track monitors are created lazily from stats (and re-created after `replaceTrack`) — an event-only sync would lose a pause that happened before the monitor existed. `DryOutboundTrackDetector` stands down on `paused`; `DryInboundTrackDetector`, `StuckDecoderDetector`, `AudioConcealmentDetector` and `JitterBufferStressDetector` stand down on either inbound flag; and both dry-track detectors resolve an already-raised dry issue the moment a pause explains the silence (previously the issue stayed open until bytes flowed again).
-   **Data channels carry their mediasoup identity.** `DATA_PRODUCER_CREATED`/`DATA_CONSUMER_CREATED` events already shipped `dataProducerId`/`dataConsumerId`, but the periodic `DataChannelStats` samples had no mediasoup identity at all. The binding now stamps `{ dataProducerId }` (data producers) and `{ dataProducerId, dataConsumerId }` (data consumers) onto the matching `DataChannelMonitor.attachments`, joining mediasoup's `sctpStreamParameters.streamId` against `RTCDataChannelStats.dataChannelIdentifier` — retried each stats tick until the lazily-created monitor appears, then the watcher detaches.
...
-   **Normalized 0–1 penalty ramps replaced the stepped subtractions** for the metric-driven reasons: nothing is subtracted at or below an *activation threshold*, the penalty ramps linearly and saturates at `1.0` at a *saturation point* (`clamp((value − activation) / (saturation − activation), 0, 1)`), and the reason key only appears once the penalty is above 0. Normalized this way: `volatile-fps` (0.1 → 0.2 volatility), `dropped-video-frames` (10% → 20% dropped), `video-frame-corruptions` (0.05 → 0.5 per-interval corruption probability, previously an unthresholded `2 × probability`), `low-bitrate-for-resolution` (codec floor → ½ of the floor), `high-deviation-from-target-bitrate` (5% → 15% under target, absolute `max(20 kbps, 5%)` gate kept) and `high-volatile-bitrate` (0.1 → 0.2). Effectively-binary penalties stay stepped (`frozen-video`, `low-fps`, `cpu-limitation`, `bandwidth-limitation`, `downscaled-screenshare`, the PC-level tiers). All activation/saturation points are `public static readonly` constants on `DefaultScoreCalculator`.
-   **Inbound video tracks are now penalized for their own jitter** (`high-jitter`, normalized 0–1): free below one sampling interval — 20 ms at the 90 kHz video clock, which the receiver absorbs by design — and saturating at 100 ms. The peer-connection-level `high-jitter` tiers are unchanged; the same key now simply appears on both entities.
-   **Audio scores reuse the detectors' verdicts — as gates, scaled by the live metric.** When the audio detectors run, their windowed, hysteresis-guarded issues decide *whether* a penalty applies, and the per-tick metric scales *how much*, normalized 0–1 from the detector's own configured threshold: active `audio-concealment` scaled by `concealmentRate` (`onThreshold` → 0.10), `audio-jitter-buffer-stress` → `high-jitter-buffer-delay` scaled by `jitterBufferTargetDelayInMs` (`targetDelayThresholdInMs` → 500 ms), `audio-desync` → `audio-time-stretch` scaled by `timeStretchRate` (`fractionalCorrectionAlertOnThreshold` → 0.3). A tick where the metric dipped back under the threshold contributes no penalty even while hysteresis keeps the issue open (previously an active issue always subtracted a fixed −2/−1/−1). Without the detectors the score falls back to a pure loss decay. The decay itself moved to the rate-independent `deltaFractionLost` (outbound audio too, replacing an absolute packet count) and is now **captured as a track-level `high-packetloss` reason**, so a loss-degraded track sample explains itself.
-   **Inbound video** gained `frozen-video` (−2, from the freeze state the detector derives), `low-fps` (−1 at ewma < 10 fps, only while frames are flowing) and `low-bitrate-for-resolution` (normalized 0–1 against the codec's `BPP_RANGES` floor — blur and blockiness long before anything freezes).
-   **Inbound screen share is now declarable, and scored accordingly.** `InboundTrackMonitor` mirrors the outbound content-type mechanism: an optional `contentType` (`'camera' | 'screenshare'`, exported as `TrackContentType`), `setContext()` and `isScreenShare`. A received track exposes no `displaySurface` to auto-detect from, so the application declares it — `monitor.getInboundTrackMonitor(track.id)?.setContext({ contentType: 'screenshare' })`. Inbound screen-share tracks skip the frame-rate penalties (`low-fps`, `volatile-fps`): mostly-static content legitimately runs at very low and bursty frame rates.
...
-   **Score reasons ship — magnitudes included, and controllable.** The sample schema's `scoreReasons` is a `Record<string, number>`, and the client, peer-connection and track sample entries now carry the score reasons attached to that entity's calculated score together with the points each one subtracted (previously the field was always empty — the encoding hooks were never implemented). Attribution is per entity by construction: track-caused reasons ship on the track's own sample, the peer connection's on its own, the client aggregate on the client sample; the sample gets a copy, never an alias of the live reasons object the calculator replaces per tick. The `encode*ScoreReasons` methods were removed from the `ScoreCalculator` interface — custom calculators only implement `update()` and set `reasons` on the calculated scores; the sampling helper is exported as `sampledScoreReasons`. The new **`sendScoreReasonsToServer`** config flag (default `true`; only an explicit `false` disables) drops the reasons from the wire without touching the scores or the realtime `'score'` event.
...
-   New score reason keys servers may encounter: `high-jitter`, `bandwidth-limitation`, `frozen-video`, `low-bitrate-for-resolution`, `audio-concealment`, `audio-time-stretch`, `high-jitter-buffer-delay`, `downscaled-screenshare`.
-   New docs: `docs/SCORE_CALCULATIONS.md` (the full scoring reference — every reason key, threshold, ramp and formula of the `DefaultScoreCalculator`, with a per-reason table of what each key means for the user experience; the README's Score Calculation section is now the overview and links to it).
