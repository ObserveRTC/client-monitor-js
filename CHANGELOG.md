...
-   **The client score is one weighted average over peer connections and tracks as siblings.** It previously computed `weightedTrackScore × (pcScore / 5)`, applying a path penalty once inside each track score and again as a factor over all of them. Weights unchanged: peer connection 1, audio track 1, video track 2.
-   **RTT and jitter are separate peer-connection reasons** — a long path and a jittery path are different problems (`high-rtt`/`very-high-rtt`; `high-jitter` at −1 above 30 ms, −2 above 100 ms). Loss uses the per-interval `deltaFractionLost` and is **averaged** across streams rather than summed.
