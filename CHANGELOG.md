...
### Pre-declared track content types

-   **`ClientMonitor.setTrackContentType(trackId, contentType)`.** Declares a track's content type (`'camera'` / `'screenshare'`) by track id, whether or not the track's monitor exists yet. `getInbound/OutboundTrackMonitor(id)?.setContentType(...)` requires the monitor to already exist, which only happens on the first stats tick after the track appears on a peer connection — but signaling often announces a guest's upcoming screen-share track before any media arrives, and the declaration was lost. Now it is applied immediately when the monitor exists, and otherwise kept as a pending entry (newest per track id wins) that whichever peer connection first manifests the track consumes at track-monitor creation. No timers, no cleanup.
...
-   **Inbound screen share is now declarable, and scored accordingly.** `InboundTrackMonitor` mirrors the outbound content-type mechanism: an optional `contentType` (`'camera' | 'screenshare'`, exported as `InboundTrackContentType`), `setContentType()` and `isScreenShare`. A received track exposes no `displaySurface` to auto-detect from, so the application declares it — `monitor.getInboundTrackMonitor(track.id)?.setContentType('screenshare')`. Inbound screen-share tracks skip the frame-rate penalties (`low-fps`, `volatile-fps`): mostly-static content legitimately runs at very low and bursty frame rates.
