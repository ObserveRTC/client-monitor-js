...
-   **Inbound video** gained `frozen-video` (−2, from the freeze state the detector derives), `low-fps` (−1 at ewma < 10 fps, only while frames are flowing) and `low-bitrate-per-pixel` (normalized 0–1 against the codec's `BPP_RANGES` floor — blur and blockiness long before anything freezes).
