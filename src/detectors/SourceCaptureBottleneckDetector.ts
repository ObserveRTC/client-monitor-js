...
export type SourceCaptureBottleneckDetectorConfig = {
	/**
 ...
 * `EncoderPerformanceDetector`, the decoder to `DecoderBottleneckDetector`.
 *
 ...
 *
 * "Measured elapsed time" is `mediaSource.deltaTime` — the gap between the two media
 * source reports, which is the same interval `sourceFps` was itself derived over. A
 * collection running late is not evidence about the camera, and taking the window from
 * the wall clock instead would silently turn it into exactly that.
 *
 ...
 * Config: `sourceCaptureBottleneckDetector`.
 *
 * Category: Pipeline Disruption
 * Layer: Send — capture to frame supply
 *
 */
export class SourceCaptureBottleneckDetector implements Detector {
	public static readonly ISSUE_TYPE = 'capture-bottleneck';

	public readonly name = 'source-capture-bottleneck-detector';
	...
	/** Stats time gathered into the current window, accumulated from `mediaSource.deltaTime`. */
	private _windowInMs = 0;
	/**
	 * False until a tick has been seen since the last stand-down. The first tick back only opens
	 * the window: its `deltaTime` spans the stretch that was deliberately not judged — a paused
	 * sender, a backgrounded tab — and folding that into the average would blame the camera for it.
	 */
	...
	public constructor(
		...
		this._issueKey = `${SourceCaptureBottleneckDetector.ISSUE_TYPE}-track-${trackMonitor.track.id}`;
	}

	private get config(): SourceCaptureBottleneckDetectorConfig {
		return this.peerConnection.parent.config.sourceCaptureBottleneckDetector!;
	}
	...
	public update() {
		...
		// The measured gap between the two media-source reports `sourceFps` was itself derived
		// from, so the frames and the seconds they are spread over come from one clock.
		const elapsedInMs = mediaSource?.deltaTime;
		...
		this._framesInWindow += sourceFps * (elapsedInMs / 1000);
		this._windowInMs += elapsedInMs;

		if (this._windowInMs < this.config.durationInMs) return;

		const averageFps = this._framesInWindow / (this._windowInMs / 1000);
		...
		clientMonitor.raiseIssue<CaptureBottleneckIssuePayload>(this._issueKey, {
			...
			type: SourceCaptureBottleneckDetector.ISSUE_TYPE,
			payload: {
